...
export default function DashboardCompras() {
  ...
  const [anio, setAnio] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('dashboardComprasAnio');
  ...
  const [mes, setMes] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('dashboardComprasMes');
  ...
  const cargarDatos = async () => {
    try {
      ...
      const fechaInicio = toLocalYYYYMMDD(new Date(anio, mes, 1));
      const fechaFin = toLocalYYYYMMDD(new Date(anio, mes + 1, 0));
      // Usar el token del contexto de autenticación
      const url = `${process.env.NEXT_PUBLIC_API_URL}/api/dashboard/compras?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
  ...
  const handleMesChange = (nuevoMes) => {
    setMes(nuevoMes);
    localStorage.setItem('dashboardComprasMes', nuevoMes.toString());
  };

  // Función para actualizar año y guardar en localStorage
  const handleAnioChange = (nuevoAnio) => {
    setAnio(nuevoAnio);
    localStorage.setItem('dashboardComprasAnio', nuevoAnio.toString());
  };
  ...
  const { kpis, comprasDiarias, topProductos } = datos || {};

  return (
    <div className="w-full mx-auto mt-10 pr-4 mr-8">
      {/* Título y selector de mes alineados como header de órdenes */}
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-800 mt-[-15px]">Dashboard de Compras</h1>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 w-full flex flex-col gap-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KPI label="Compra diaria promedio" value={formatCurrency(kpis?.promedioDiario || 0)} />
          <KPI label="Compra del mes" value={formatCurrency(kpis?.totalCompras || 0)} />
          <KPI label="Compra del día" value={formatCurrency(kpis?.compraHoy || 0)} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-72">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col">
            <div className="text-base font-semibold text-blue-700 mb-2">Compras diarias</div>
            <div className="flex-1">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={comprasDiarias || []} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
